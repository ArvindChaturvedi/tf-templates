...
module "team1_db_security" {
  ...
  db_port               = var.db_port
  allowed_cidr_blocks   = concat([data.aws_vpc.existing.cidr_block], var.team1_allowed_cidrs)
...
module "team2_db_security" {
  ...
  create_security_group = true
  db_port               = var.db_port
  allowed_cidr_blocks   = concat([data.aws_vpc.existing.cidr_block], var.team2_allowed_cidrs)
